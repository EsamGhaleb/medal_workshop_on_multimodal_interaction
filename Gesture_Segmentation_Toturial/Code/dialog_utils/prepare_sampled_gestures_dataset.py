...
def read_manually_segmented_gestures():
    all_gestures_info = []
    elan_files_path = os.path.expanduser('/data/clusterfs/mld/users/esagha/data/ElanFiles//New_ELAN_files/')
...
if __name__ == "__main__":
    gestures_path = "/data/clusterfs/mld/users/esagha/Projects/gestural_represenation/data/segmented_gestures/segmented_gestures.pkl"
    gestures_info = pd.read_pickle(gestures_path)
    # if pair_speaker contains 'pair' then it is a manual segmentation
    gestures_info['segmentation method'] = gestures_info['pair_speaker'].apply(lambda x: 'manual' if 'pair' in x else 'automatic')
    ...
    for a_speaker in tqdm(unique_speakers):
        ...
        audio_path = os.path.join("/data/clusterfs/mld/users/esagha/data", f"{pair}_synced_pp{speaker}.wav")
    ...
    for i, row in tqdm(gestures_info.iterrows(), total=len(gestures_info)):
        ...
        if not speech_info.empty:
            ...
            print("Automatic transcription for ", pair_speaker)
            try:
                sr = 16000
    ...
    gestures_info.to_pickle('/data/clusterfs/mld/users/esagha/Projects/gestural_represenation/data/segmented_gestures/segmented_gestures_with_text.pkl')