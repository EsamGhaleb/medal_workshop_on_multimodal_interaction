...
from feeders.audio_video_test_feeder import SequentialAudioSkeletonTestFeeder as TestFeeder
...
def main(lr=1e-4):
    ...
    sequential_audio_dataset = SequentialAudioSkeletonFeeder(**arg.feeder_args)
    if arg.phase == 'test':
        # label_path: data/large_cabb_data/27_labels_buffer_0.0.pkl
        # data_path: data/large_cabb_data/data_27_joint_buffer_0.0.npy
        arg.feeder_args['data_path'] = arg.feeder_args['data_path'] = 'data/large_cabb_data/data_27_joint_buffer_0.0.npy'
        arg.feeder_args['label_path'] = arg.feeder_args['label_path'] = 'data/large_cabb_data/27_labels_buffer_0.0.pkl'
    ...
    sample_names = sequential_audio_dataset.sample_name
    # extract the speaker_pair from the labels
    speaker_ID = np.array([elem[0].split("_")[0]+'_'+elem[0].split('_')[1] for elem in sample_names])
    gkf = GroupKFold(n_splits=5)
    if arg.feeder_args['subject_joint']:
        splitting_method = kf.split(sequential_audio_dataset)
    else:
        splitting_method = gkf.split(sequential_audio_dataset, groups=speaker_ID)
    ...
    for fold, (train_index, test_index) in enumerate(splitting_method):
        ...
        sequential_audio_dataset.fold = fold
        arg.feeder_args['fold']=fold
        arg.feeder_args['vggish'] = arg.fusion_args['vggish']
        arg.feeder_args['speech_buffer'] = arg.fusion_args['speech_buffer']
        ...
        train_dataset = torch.utils.data.Subset(sequential_audio_dataset, train_index)
        test_dataset = torch.utils.data.Subset(sequential_audio_dataset, test_index)
        ...
        sums = [1 if sum(label)> 0 else 0 for _, _, label, _, _ in train_dataset]   
        train_dataset = WrapperDataset(train_dataset, augmentation_apply=True)
        ...
        # Model 
        logger_name = arg.Experiment_name
        ...
        if arg.phase == 'train':
            ...
            trainer.save_checkpoint(models_directory+arg.Experiment_name+"/last_epoch.ckpt")
        elif arg.phase == 'test':
    ...
    if arg.phase == 'train':
        ...
        results_path = models_directory+arg.Experiment_name+"/test_results.pkl"
