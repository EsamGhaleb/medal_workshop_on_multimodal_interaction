...
def load_data(keypoints_path, config):
    ...
    for file in tqdm(glob.glob(os.path.join(poses_path, '*.npy')), desc='Loading keypoints...', total=len(glob.glob(os.path.join(poses_path, '*.npy')))):
        ...
        file_ID = file.split('/')[-1].replace('_selected_keypoints.npy', '').replace('results_', '').split('_')[0]
        type = file.split('/')[-2]
        ID_type = file_ID+'+'+type
        ...
        if data.shape[0] == 0:
            ...
            continue
        data_dict[(file_ID, type)] = np.load(file)
...
if __name__ == '__main__':
    ...
    out_folder = project_directory+'data/dilay_data/'  
