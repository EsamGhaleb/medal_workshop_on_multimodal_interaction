...
def load_data(keypoints_path, config):
    # ../gestural_alignment/data/final_poses/*.npy
    data_dict = dict()
    for i, path in tqdm(enumerate(glob.glob(keypoints_path)), leave=True):
        if 'pair' in path:
            continue
        pair = path.split('/')[-1].split('_')[1]
        speaker = path.split('/')[-1].split('_')[-1].split('.')[0][-1]
        data_dict[(pair, speaker)] = np.load(path)[:, selected_joints[config], :]
    return data_dict
...
if __name__ == '__main__':
    ...
    parser.add_argument('--keypoints-path', type=str, help='The path to the numpy file', default='../gestural_alignment/data/final_poses/*.npy')
    ...
    out_folder = project_directory+'data/large_cabb_data/'  
